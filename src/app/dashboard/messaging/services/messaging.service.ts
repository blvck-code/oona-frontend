...
import { getPrivateMessages } from '../../../dashboard/state/entities/messages/private.messages.entity';
...
export class MessagingService {
  ...
  handleUnreadPrivateMessages(): void {
    ...
    this.store.select(getPrivateMessages).subscribe((messages: any) => {
      messages.map((message: SingleMessageModel) => {
  ...
  handleUnreadStreamMessages(): void {
    ...
    this.store.select(getStreamMessages).subscribe((messages: any) => {
  ...
  handleGetStreamMessages(): void {
    this.store.select(getStreamMessages).subscribe((streams: any) => {
      streams?.map((stream: any) => {
        ...
        this.store.dispatch(
          new messagingActions.LoadStreamMessage(streamDetail)
        );
      });
    });
  }
  ...
  handleGetPrivateMessages(): void {
    ...
    this.store.select(getZulipUsers).subscribe((users: any) => {
      users?.map((user: any) => {
        const streamDetail = {
          anchor: 'newest',
          num_before: 100,
        ...
        this.store.dispatch(
          new messagingActions.LoadPrivateMessages(streamDetail)
        );
      });
    });
  }

  updateReadMessagesFlags(
    messageType: string,
    unreadMsgIds: number[]
  ): Observable<any> {
    this.privateUnreadMsgArrayObservable.subscribe(
      (messages: SingleMessageModel[]) => {
        unreadMsgIds.map((id: number) => {
          const newMessages = messages.filter(
            (message: SingleMessageModel) => message.id !== id
          );
    ...
    return this.http.post(env.updateMessageFlag, request);
  }
