...
export class IndividualMessagingRightPanelComponent implements OnInit {
  ...
  updateProfile(): void {
    this.messagingService
      .getOonaMemberDetail(this.memberDetails.email)
      .subscribe((oonaProfileData: { results: any }) => {
        if (oonaProfileData.results[0]) {
          this.oonaProfile = oonaProfileData.results[0];
          this.profileCreationDate = new Date(
            oonaProfileData.results[0].created_at
          );
        } else {
      ...
      .subscribe((subscribers: { subscriptions: any; name: any }) => {
        ...
        this.commonTeams = subscribers.subscriptions.filter(
          (team: { subscribers: any[] }) =>
            team.subscribers?.includes(this.memberDetails.email) &&
            team.subscribers?.includes(this.loggedInUserProfile.email)
        );
  ...
  generateMeeting(): void {
    ...
    this.messagingService
      .getOonaMemberDetail(this.currentUser.email)
      .subscribe((oonaProfileData: { results: any }) => {
        const meetingAttendees = [];
        if (oonaProfileData.results.length < 1) {
          this.notificationService.showWarning(
            `${this.currentUser.full_name} is not a member of oona`,
          ...
          this.messagingService
            ...
            .subscribe((response: any) => {
              ...
              this.notificationService.showInfo(
                `Meeting with ${this.currentUser.full_name} has been created.`,
                'Meeting created'
              );
              setTimeout(() => {
