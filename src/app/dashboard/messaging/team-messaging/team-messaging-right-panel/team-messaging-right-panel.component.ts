...
export class TeamMessagingRightPanelComponent implements OnInit {
  ...
  getDetails(streamId: string): void {
    ...
    this.messagingService.getAllSubscribedStreams().subscribe(
      ( subscribers: { subscriptions: any; name: any; }) => {
        this.allSubscribers = subscribers.subscriptions;
        // tslint:disable-next-line:max-line-length
        const allEmailsSubscribed = subscribers.subscriptions.find((teams: { stream_id: any; }) => teams.stream_id === Number(streamId)).subscribers;
        const streamName = subscribers.subscriptions.find((teams: { stream_id: any; }) => teams.stream_id === Number(streamId)).name;
        ...
        this.messagingService.changeCurrentStreamMemberNames(this.groupMemberDetails);
      });
  ...
  compare( member1: any, member2: any ): number {
    ...
    if ( member1.presence.website.status === 'active' && member2.presence.website.status === 'idle' ){
      return -1;
    }
    // // place second member before first
    if ( member1.presence.website.status === 'idle' && member2.presence.website.status === 'active' ){
      return 1;
    }
