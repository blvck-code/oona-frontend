...
export class TeamMessagingComponent implements OnInit {
  ...
  selectedStream$: Observable<number | null> =
    this.store.select(getSelectedStream);
  ...
  updateMessageFlags(): void {
    ...
    this.messageUpdate$.subscribe((messages: SingleMessageModel[]) => {
      messages.map((message: SingleMessageModel) => {
        ...
        this.messageSrv
          ...
          .subscribe((response: any) => {
            ...
            this.store.dispatch(
              new messagingActions.UpdateReadMessageSuccess(messageId)
            );
          });
